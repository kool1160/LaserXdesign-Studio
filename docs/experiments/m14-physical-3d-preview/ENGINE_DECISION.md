...
- `assembledDepthMm` is the sum of every declared physical layer's exact `thicknessMm` plus `assembledGapMm` between them — distinct from any exploded-mode presentation spread. It is **not** automatically "real physical stack depth": it always includes a layer's declared thickness even when that layer rendered no geometry. Whether it may be trusted as verified finished depth is a separate field, `depthStatus` (`"verified" | "declared-incomplete" | "unavailable"`, corrected 2026-08-02 — see the second "Repair" section below).
...
## Repair — partial assembly overstated declared thickness as verified depth (2026-08-02)

**Defect:** the previous repair correctly made an empty physical layer produce a visible finding and a `"partial"` assembly status, but `assembledDepthMm` still summed every declared layer's `thicknessMm` regardless of whether it rendered, the browser continued labeling that number `total assembled depth` in every status, and this document itself called `assembledDepthMm` "the real physical stack depth" in the same sentence that acknowledged an empty layer contributes to it without rendering. A user could see the warning that a layer has no geometry and, immediately next to it, a precise-looking finished depth that includes that nonexistent solid — the numeric label contradicted the warning.

**Fix — smallest compatible contract, per the review's preferred option:**

- `assembledDepthMm` is unchanged in meaning and calculation (backward compatible: still the sum of every declared physical layer's `thicknessMm` plus `assembledGapMm` between them) — it was never wrong as a *number*, only as an unqualified claim of verified depth.
- Added `PhysicalPreviewAssemblyDepthStatus = "verified" | "declared-incomplete" | "unavailable"` and a new `depthStatus` field on `PhysicalPreviewAssembly`, mapped one-to-one from `status` (`"complete"` → `"verified"`, `"partial"` → `"declared-incomplete"`, `"unavailable"` → `"unavailable"`). `assembledDepthMm` may only be presented as verified/real/exact/total finished depth when `depthStatus === "verified"`.
- The browser (`App.tsx`) now derives its depth label from `depthStatus`: `"total assembled depth"` only when `"verified"`; `"declared stack depth — incomplete"` for both `"declared-incomplete"` and `"unavailable"` (the unavailable case never reaches this code path since the app renders a distinct "no physical layers" state first). The words `exact`, `real`, `verified`, and `total assembled depth` never appear together with a `"declared-incomplete"` depth.
- Empty/ambiguous layers still keep their declared identity, material, thickness, document order, and presentation Z slot (`assembledZRangeMm`/`explodedZRangeMm`) — documented as declared placement (the space the layer *would* occupy), not a rendered solid extent. No geometry is invented for them.
