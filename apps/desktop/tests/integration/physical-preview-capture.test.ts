...
import type { PreviewCaptureDecoderPort } from "../../electron/preview-capture-decoder.js";
...
/** IDAT, an ancillary chunk, then another IDAT: every chunk is well-framed
 * and CRC-correct, but PNG requires IDAT chunks to be consecutive. */
function nonConsecutiveIdatBase64(): string {
  const signature = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
  const ihdrData = Buffer.alloc(13);
  ihdrData.writeUInt32BE(CAPTURE_WIDTH, 0);
  ihdrData.writeUInt32BE(CAPTURE_HEIGHT, 4);
  ...
  return Buffer.concat([
    ...
    chunk("IHDR", ihdrData),
    chunk("IDAT", deflateSync(raw)),
    chunk("tEXt", Buffer.from("Comment\0hello", "ascii")),
    chunk("IDAT", deflateSync(raw)),
    chunk("IEND", Buffer.alloc(0)),
  ]).toString("base64");
}

/** Valid signature, IHDR, chunk CRCs, and IEND -- but the IDAT payload is
 * not a valid deflate stream, so no real decoder can produce an image. */
function undecodableIdatBase64(): string {
  const signature = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
  const ihdrData = Buffer.alloc(13);
  ihdrData.writeUInt32BE(CAPTURE_WIDTH, 0);
  ihdrData.writeUInt32BE(CAPTURE_HEIGHT, 4);
  ...
  return Buffer.concat([
    signature,
    chunk("IHDR", ihdrData),
    chunk("IDAT", Buffer.from("this is not a deflate stream", "ascii")),
    chunk("IEND", Buffer.alloc(0)),
  ]).toString("base64");
}
...
/**
 * Genuinely decodes: concatenates every IDAT payload and inflates it, so a
 * structurally perfect stream carrying garbage compressed data is rejected
...
async function desktop(
  ...
  const controller = new DesktopController({
    ...
    ...(options.omitDecoder === true
      ? {}
      : { previewCaptureDecoder: inflatingDecoder }),
  });
...
describe("privileged physical preview capture save", () => {
  ...
  it("rejects non-consecutive IDAT chunks without prompting or writing", async () => {
    const harness = await desktop((directory, suggested) => join(directory, suggested));

    await harness.controller.savePhysicalPreviewCapture(
      request(harness, { pngBase64: nonConsecutiveIdatBase64() }),
    );

    expect(harness.controller.state.physicalPreview.capture?.error).toMatch(
      /non-consecutive-idat/u,
    );
    expect(harness.chosenPaths).toEqual([]);
    expect((await readdir(harness.directory)).filter((n) => n.endsWith(".png"))).toEqual([]);
  });

  it("rejects a structurally valid PNG whose compressed data cannot be decoded", async () => {
    const harness = await desktop((directory, suggested) => join(directory, suggested));

    await harness.controller.savePhysicalPreviewCapture(
      request(harness, { pngBase64: undecodableIdatBase64() }),
    );
  ...
  it("rejects a capture when no decoder is wired, failing closed", async () => {
    const harness = await desktop(
      (directory, suggested) => join(directory, suggested),
      { omitDecoder: true },
    );
